...
fn interpretModifier(byte: u8, key_press: *ku.KeyPress) !void {
    // print("interpretModifier {any}\n", .{byte});
    if (byte < 1 or byte > 16) {
        if (byte > 48) { //    subtract '0'
            return interpretModifier(byte - 48, key_press);
        } else return ParseModiferError.ValueError;
    }
    // modifier bitmap
    const bmap: u8 = byte - 1;
    if (bmap & 1 != 0) key_press.shift = true;
    if (bmap & 2 != 0) key_press.alt = true;
    if (bmap & 4 != 0) key_press.ctrl = true;
    // if (bmap & 8 != 0) key_press.meta = true;
...
fn parseInputBytes(bytes: []const u8, key_press: *ku.KeyPress) !void {
    ...
    switch (bytes.len) {
        ...
        1 => {
            switch (bytes[0]) {
                0...127 => |byte| key_press.Key = .{ .ascii = byte },
                else => {
                    return ParseError.ParseBase;
                },
            }
        ...
        2 => {
            ...
            key_press.alt = true;
            return parseInputBytes(bytes[1..], key_press);
        },
        else => {
            if (std.mem.eql(u8, bytes[0..2], &.{ '\x1B', 'O' })) {
                // SS3 (Single Shift 3) sequences '\x1B O <keycode>'
                return match_keycodes.match_Ss3(bytes[2..], key_press);
            }
            if (bytes.len >= 6 and std.mem.eql(u8, bytes[0..4], &.{ '\x1B', '[', '1', ';' })) {
                // CSI 1 ; <modifier> <keycode>
                // xterm sequence with modifier
                try interpretModifier(bytes[4], key_press);
                return match_keycodes.matchXterm(bytes[5..], key_press);
            }
            ...
            if (bytes[bytes.len - 1] == '~') {
                // vt sequence
                // [esc] '[' ([keycode]) (';'[modifier]) '~'
                try processVtSequence(bytes[2..(bytes.len - 1)], key_press);
            } else {
                if (bytes[2] == '[') {
                    // older vt sequence
                    try match_keycodes.matchOldSchemeVt(bytes[2..], key_press);
                } else {
                    // xterm sequence
                    // [esc] '[' ([modifier]) [char]
                    // the terminal seems to ignore modifiers,
                    //  and so we ignore them
                    try match_keycodes.matchXterm(bytes[2..], key_press);
                }
            }
        },
    }
